...
import { EnrichedProducts, CustomDesignDocument, ItemDocument } from "@/types/types";
import { getMostRecentCustomDesign } from "@/app/actions";
import LogButton from "@/components/test/LogButton";
...
export async function generateMetadata() {
  return {
    title: "Cart | Zesty Merch",
    description: `Cart at Zesty Merch`,
  };
}
...
const ProductsCart = async ({ session }: { session: Session }) => {
  const calculateTotalPrice = (cart: EnrichedProducts[]) => {
    ...
    return cart
      .reduce(
        (total: number, cartItem: EnrichedProducts) =>
  ...
  if (filteredCart) {
    for (const item of filteredCart) {
      const customDesign: CustomDesignDocument | null = await getMostRecentCustomDesign(session.user.email, item.productId.toString());
  ...
  const totalPrice = calculateTotalPrice(filteredCart as EnrichedProducts[]);
      ...
      <div className="pt-12">
        ...
        <div className="fixed left-[50%] translate-x-[-50%] bottom-4 w-[90%] z-10 sm:w-[360px] rounded-xl overflow-hidden flex bg-black border border-solid border-border-primary h-min">
          ...
          <div className="w-1/2 border-l border-solid bg-background-secondary border-border-primary">
            <ButtonCheckout session={session} cartWithProducts={filteredCart as ItemDocument[]} />
            {/* <LogButton data={filteredCart} /> */}
          </div>
        </div>
      </div>
    );

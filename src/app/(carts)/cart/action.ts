...
import { getCustomDesignById } from "@/app/actions";
...
export type Cart = {
  ...
  items: Array<{
    ...
    quantity: number;
    price: number;
    designId: Schema.Types.ObjectId | undefined;
  }>;
};
...
export async function addItem(
  ...
  let design;
  if (designId) {
    design = await getCustomDesignById(designId);
  }
  const diD: Schema.Types.ObjectId | undefined = design?._id; 

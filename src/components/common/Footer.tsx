"use client";

import React from "react";
import Link from "next/link";
import { website } from "@/constants";
import { IconBrandInstagram } from "@tabler/icons-react";

export const Footer = () => {
  const linkStyles =
    "text-sm transition duration-150 ease-in-out hover:text-white";
  const liStyles = "text-[#A1A1A1] my-1.5";

  return (
    <footer className="px-6 py-24 border-t border-solid bg-[#0A0A0A] border-[#242424]">
      <nav className="flex flex-wrap justify-around gap-10 mx-auto max-w-screen-2xl">
        <div className="flex flex-col items-center justify-center w-full max-w-xs gap-5">
          <div className="flex gap-3.5 items-center">
            <Link
              href="https://harshitkumar.tech/"
              target="_blank"
              title="Portfolio of Harshit"
              aria-label="Portfolio of Harshit"
            >
              <span className="text-sm text-[#A1A1A1]">© 2024 Zesty Merch</span>
            </Link>
          </div>
          <div className="flex gap-3.5 items-center">
            <Link
              href="https://github.com/harshitkumar9030"
              target="_blank"
              title="GitHub of Harshit"
              aria-label="GitHub of Harshit"
            >
              <svg
                height="24"
                viewBox="0 0 16 16"
                width="24"
                style={{ color: "currentColor" }}
                aria-hidden="true"
              >
                <path
                  fillRule="evenodd"
                  clipRule="evenodd"
                  d="M8 0C3.58 0 0 3.57879 0 7.99729C0 11.5361 2.29 14.5251 5.47 15.5847C5.87 15.6547 6.02 15.4148 6.02 15.2049C6.02 15.0149 6.01 14.3851 6.01 13.7154C4 14.0852 3.48 13.2255 3.32 12.7757C3.23 12.5458 2.84 11.836 2.5 11.6461C2.22 11.4961 1.82 11.1262 2.49 11.1162C3.12 11.1062 3.57 11.696 3.72 11.936C4.44 13.1455 5.59 12.8057 6.05 12.5957C6.12 12.0759 6.33 11.726 6.56 11.5261C4.78 11.3262 2.92 10.6364 2.92 7.57743C2.92 6.70773 3.23 5.98797 3.74 5.42816C3.66 5.22823 3.38 4.40851 3.82 3.30888C3.82 3.30888 4.49 3.09895 6.02 4.1286C6.66 3.94866 7.34 3.85869 8.02 3.85869C8.7 3.85869 9.38 3.94866 10.02 4.1286C11.55 3.08895 12.22 3.30888 12.22 3.30888C12.66 4.40851 12.38 5.22823 12.3 5.42816C12.81 5.98797 13.12 6.69773 13.12 7.57743C13.12 10.6464 11.25 11.3262 9.47 11.5261C9.76 11.776 10.01 12.2558 10.01 13.0056C10.01 14.0752 10 14.9349 10 15.2049C10 15.4148 10.15 15.6647 10.55 15.5847C12.1381 15.0488 13.5182 14.0284 14.4958 12.6673C15.4735 11.3062 15.9996 9.67293 16 7.99729C16 3.57879 12.42 0 8 0Z"
                  fill="currentColor"
                ></path>
              </svg>
            </Link>
            <Link
              href="https://instagram.com/_harshit.xd"
              target="_blank"
              title="Instagram of Harshit"
              aria-label="Instagram of Harshit"
            >
              <IconBrandInstagram className="w-8 h-8" />
            </Link>
          </div>
        </div>
        <div className="w-full max-w-xs">
          <h2 className="my-3 text-sm font-medium text-neutral-200">Products</h2>
          <ul className="grid grid-cols-2 gap-4">
            <li className={liStyles}>
              <Link
                href={`${process.env.NEXT_PUBLIC_APP_URL}/t-shirts`}
                className={linkStyles}
              >
                T-shirts
              </Link>
            </li>
            <li className={liStyles}>
              <Link
                href={`${process.env.NEXT_PUBLIC_APP_URL}/pants`}
                className={linkStyles}
              >
                Pants
              </Link>
            </li>
            <li className={liStyles}>
              <Link
                href={`${process.env.NEXT_PUBLIC_APP_URL}/sweatshirts`}
                className={linkStyles}
              >
                Sweatshirts
              </Link>
            </li>
            <li className={liStyles}>
              <Link
                href={`${process.env.NEXT_PUBLIC_APP_URL}/mugs`}
                className={linkStyles}
              >
                Mugs
              </Link>
            </li>
            <li className={liStyles}>
              <Link
                href={`${process.env.NEXT_PUBLIC_APP_URL}/stickers`}
                className={linkStyles}
              >
                Stickers
              </Link>
            </li>
          </ul>
        </div>
        <div className="w-full max-w-xs">
          <h2 className="my-3 text-sm font-medium text-neutral-200">Assistance</h2>
          <ul className="grid grid-cols-2 gap-4">
            <li className={liStyles}>
              <Link href="/size" className={linkStyles}>
                Size guide
              </Link>
            </li>
            <li className={liStyles}>
              <Link href="/delivery" className={linkStyles}>
                Delivery
              </Link>
            </li>
            <li className={liStyles}>
              <Link href="/refunds" className={linkStyles}>
                Returns and refunds
              </Link>
            </li>
            <li className={liStyles}>
              <Link href="/faq" className={linkStyles}>
                FAQs
              </Link>
            </li>
            <li className={liStyles}>
              <Link href="/contact" className={linkStyles}>
                Contact Us
              </Link>
            </li>
          </ul>
        </div>
        <div className="w-full max-w-xs">
          <h2 className="my-3 text-sm font-medium text-neutral-200">Zesty Merch</h2>
          <ul className="grid grid-cols-2 gap-4">
            <li className={liStyles}>
              <Link
                href={website}
                target="_blank"
                className={linkStyles}
                rel="noopener noreferrer"
              >
                Website
              </Link>
            </li>
            <li className={liStyles}>
              <Link
                href="https://instagram.com/_harshit.xd"
                target="_blank"
                className={linkStyles}
                rel="noopener noreferrer"
              >
                Instagram
              </Link>
            </li>
            <li className={liStyles}>
              <Link href="/about" className={linkStyles}>
                About Us
              </Link>
            </li>
            <li className={liStyles}>
              <Link href="/battles" className={linkStyles}>
                Design Battles
              </Link>
            </li>
            <li className={liStyles}>
              <Link href="/tos" className={linkStyles}>
                Terms of Service
              </Link>
            </li>
            <li className={liStyles}>
              <Link href="/privacy-policy" className={linkStyles}>
                Privacy Policy
              </Link>
            </li>
          </ul>
        </div>
      </nav>
    </footer>
  );
};

export default Footer;

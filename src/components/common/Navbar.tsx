...
export const Navbar = ({ session, totalItemsCart, totalWishlists }: Navbar) => {
  ...
  return (
    <header className="pointer-events-auto z-50 w-full px-3.5 gap-4 xs:px-6 sm:px-12 py-6 flex items-center justify-between bg-background-secondary border-b border-solid border-border-primary">
      <button
        onClick={() => {
          toggleHeader();
            ...
            {session?.user ? (
              <>
                <li className="flex flex-col gap-6 justify-center items-center">
                  ...
                  <Link className="flex items-center px-4 py-2 w-full h-full" onClick={toggleHeader} href="/designs">
                <svg
                  data-testid="geist-icon"
                  height="16"
                  strokeLinejoin="round"
      ...
      <ul className="justify-between items-center hidden gap-2 text-sm lg:flex">
        <Link href={'/'}>
          <HomeIcon className="h-6 w-6  rounded-lg hover:text-white duration-300 flex justify-center items-center text-center mr-2 text-gray-500" />
        </Link>
        {authLinks()}
        <li>
          <LinksDesktop />
      ...
      <ul className="flex gap-2">
        <li className="flex items-center justify-center">
          ...
          >
            ...
            <span className="flex items-center bg-purple-700 font-medium text-[#EDEDED] justify-center absolute w-[20px] rounded-full top-[-3px] right-[-3px]">
              {totalItemsCart}
            </span>
          </Link>
        </li>

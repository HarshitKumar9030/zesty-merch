...
import { Design } from "@canva/connect-api-ts/types.gen";
...
export const getDesign = async ({ designId }: { designId: string }): Promise<Design> => {
  return fetchData(endpoints.GET_DESIGN.replace(":designId", designId))
};
